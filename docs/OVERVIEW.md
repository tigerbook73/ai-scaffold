...
The Codex install manifest generated by `pnpm build:codex`. It includes only skills with `targets.codex: true` in `skills/manifest.json`.

Each entry contains:

- `src`: source path relative to `skills/`
- `name`: Codex skill name
- `dst`: future install destination under `.codex/skills/{name}/SKILL.md`
