...
| **resource**   | 文件模板等静态资源，供 skill/rule/script 引用 | 不直接安装，由其他组件使用 |
...
3. 对需要配置的组件（见"定制化"章节），系统尝试自动检测推荐值；无法自动确定时提示用户输入
4. 若有依赖未满足，自动展示并提示一并处理
5. 执行变更，报告结果
...
**核心难点**：如果定制项与主体内容混合在同一文件（如 rule guard 的 frontmatter + 正文在同一 `.md` 文件），更新时难以区分。

**需要验证**：是否有可行的文件结构设计，使定制项与可更新内容能够独立维护。
...
3. `setup` 能正确展示三种状态（未安装 / 已安装最新 / 有更新），默认对有更新的单元执行更新
4. `setup` 安装完成后，目标项目具备对应 AI 能力（skill 可调用、rule guard 生效）
5. `setup` 卸载后，对应能力移除，相关文件不再存在
6. `setup` 更新后，组件更新至新版本，用户定制配置保留
