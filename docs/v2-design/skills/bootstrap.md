...
初始化 AI Scaffold skill 管理器，完成后即可通过 `/aisk:skill` 命令按需安装各 skill。
...
**约束**
- [写操作] 只写 `.claude/commands/aisk/skill.md` 和 `.claude/settings.json`
...
3. 创建目录（若不存在）：
   ...
   - `.ai-skills/skills/`
4. 下载 `{AISC_REGISTRY}/skill.md` 到 `.claude/commands/aisk/skill.md`
5. 读取 `.claude/settings.json`（不存在则创建空对象），在 `env` 中写入 `AISC_REGISTRY`
6. 输出初始化摘要：已创建的目录和文件、`AISC_REGISTRY` 值，提示通过 `/aisk:skill list` 查看可用 skill
