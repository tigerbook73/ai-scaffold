...
- **plan design / refresh design** — generate or update `design.md` from requirements; must break work into numbered steps; each step must include: **Step Type** (`intermediate` — transitional code, note which step finalizes it; or `final` — production-quality code and tests required), **Auto Verification** section (`final` steps: at least one functional test command for the new behavior, not just the existing suite; `intermediate` steps: optional), **Manual Verification** section (high-cost checks that cannot be automated cheaply; tag with `[automation-candidate]` if worth automating later); sync task-state.md: Design Phase status → `in_progress`, Current Phase → `design (in_progress)`, add step entries to Implementation Phase each with `step-type` set
