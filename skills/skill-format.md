...
本文档定义代码库 skill 文件的规范源格式。Agent 专用规则文件可引用本文档，但本文档是生成 skill 格式资源的唯一事实来源。

## 文件位置

SK 文件必须直接放在 `skills/*/` 下（仅一层深度）。位于 `skills/SK-*.md` 或 `skills/*/resource/SK-*.md` 的文件属于错误放置，不应被视为有效的 skill 命令。
