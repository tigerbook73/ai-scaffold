...
// units/walkthrough/scripts/walkthrough-state.ts
...
// units/walkthrough/scripts/walkthrough-state.ts
var BASE_DIR = (0, import_path.join)(process.cwd(), ".aisk", "walkthrough", "state");
var WalkthroughState = class {
  /** Directory that owns all state files for a sanitized walkthrough key. */
  stateDir(key) {
    return (0, import_path.join)(BASE_DIR, key);
  }
  /** Absolute path to the persisted index for a state key. */
  indexPath(key) {
    return (0, import_path.join)(this.stateDir(key), "index.json");
  }
  /** Read the index file; callers handle command-specific missing-state behavior. */
  readIndex(key) {
    return JSON.parse((0, import_fs.readFileSync)(this.indexPath(key), "utf-8"));
  }
  /** Initialize a new state record from JSON produced by the walkthrough planner. */
  cmdInit(key, indexJson) {
    const dir = this.stateDir(key);
    (0, import_fs.mkdirSync)(dir, { recursive: true });
    const index = JSON.parse(indexJson);
    (0, import_fs.writeFileSync)(this.indexPath(key), JSON.stringify(index, null, 2) + "\n", "utf-8");
  }
  ...
  cmdRead(key) {
    ...
    process.stdout.write((0, import_fs.readFileSync)(path, "utf-8"));
  }
  /** Replace an existing index after the skill updates derived state. */
  cmdUpdate(key, indexJson) {
    const path = this.indexPath(key);
    if (!(0, import_fs.existsSync)(path)) {
      console.error(`No state found for key: ${key}`);
      process.exit(1);
    ...
    const index = JSON.parse(indexJson);
    (0, import_fs.writeFileSync)(path, JSON.stringify(index, null, 2) + "\n", "utf-8");
  }
  /** List compact summaries for every persisted walkthrough state. */
  cmdList() {
    if (!(0, import_fs.existsSync)(BASE_DIR)) {
      process.stdout.write("[]\n");
      return;
    }
    const entries = (0, import_fs.readdirSync)(BASE_DIR, { withFileTypes: true }).filter((d) => d.isDirectory()).map((d) => d.name);
    const results = entries.filter((key) => (0, import_fs.existsSync)(this.indexPath(key))).map((key) => {
  ...
  cmdFind(hash) {
    ...
    for (const key of entries) {
      if (!(0, import_fs.existsSync)(this.indexPath(key))) continue;
      const index = this.readIndex(key);
  ...
  run() {
    ...
    cli.command("init", "Initialize state directory and write index.json").option("--key <key>", "State key (sanitized branch name)").option("--index <json>", "Index as a JSON string").action((options) => {
      this.cmdInit(options.key, options.index);
    });
    cli.command("read", "Print index.json for a key to stdout").option("--key <key>", "State key").action((options) => {
      this.cmdRead(options.key);
    });
    cli.command("update", "Overwrite index.json for a key with new JSON").option("--key <key>", "State key").option("--index <json>", "Replacement index as a JSON string").action((options) => {
      this.cmdUpdate(options.key, options.index);
    });
    cli.command("list", "List all walkthrough states with their status summary").action(() => {
      this.cmdList();
    });
    cli.command("find", "Find an active state by target commit hash; exits 1 if not found").option("--hash <hash>", "Commit hash to search for").action((options) => {
      this.cmdFind(options.hash);
    });
    cli.command("next", "Mark current group done and print next group file to stdout").option("--key <key>", "State key").action((options) => {
      this.cmdNext(options.key);
    });
    cli.command("prev", "Go back to the previous group and print it to stdout").option("--key <key>", "State key").action((options) => {
      this.cmdPrev(options.key);
    });
    cli.command("goto", "Jump to group N and print it to stdout").option("--key <key>", "State key").option("--n <n>", "Target group number (1-based)").action((options) => {
      this.cmdGoto(options.key, Number(options.n));
    });
    cli.command("finish", "Mark walkthrough as completed (does not delete state)").option("--key <key>", "State key").action((options) => {
      this.cmdFinish(options.key);
    });
    cli.command("delete", "Delete the entire state directory for a key").option("--key <key>", "State key").action((options) => {
